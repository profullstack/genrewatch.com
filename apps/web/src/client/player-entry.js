...
function attach(video, url, onError, onNotice = () => {}) {
  ...
  function start() {
    ...
    player.on(mpegts.Events.ERROR, (type, detail, info) => {
       ...
       * entry they want now. Taking a stranger's film off them because you clicked
       * something is a different act, so the shared route refuses and this is the
  ...
  /*
   * Breaks are NOT attached here.
   *
   * They used to be, and it left the film path with none: a stored file is handed
   * straight to the element and never loads this bundle at all -- that is the
   * whole reason the two paths exist, because an iPhone has no Media Source and
   * plays the MP4 natively. A break attached inside this function reached live
   * channels only, and a name's own page, which is what a reader arrives on from a
   * search engine, plays a film.
   *
   * So the break machinery moved to vendor-ads.js, which app.js loads on the same
   * press for both paths. That also puts the interval and the creative source in
...
window.__genrePlayer = { supported, attach };
